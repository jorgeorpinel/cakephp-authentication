 ...
 * @todo use Session::setFlash() instead or additionally to Session::flash()
 */
...
//	  $succeded;
	  ...
	  $options = array(
	    ...
	    'modelName' => isset($loginOptions['modelName'])?$loginOptions['modelName']:$loginOptions['userModelPtr']->name, // XXX will fail if userModelPtr failed
	  ...
	  $username = trim($data[$options['modelName']][$options['username']]);
	  $password = trim($data[$options['modelName']][$options['password']]);
    ...
    if($succeded)
      $this->controller->Session->write($options['userId'], $user[$options['modelName']]['id']);

    // If failed & render view:
    if(!$succeded && $options['failed']['renderView']) {
      ...
      // calls post-login (failed) function:
